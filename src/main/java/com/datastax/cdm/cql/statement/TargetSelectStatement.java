...
public abstract class TargetSelectStatement extends BaseCdmStatement {
    ...
    public TargetSelectStatement(IPropertyHelper propertyHelper, EnhancedSession session) {
    ...
    public boolean  shouldFilterRecord(Record record) {
        ...
        if (this.filterColumnEnabled) {
            String col = (String) cqlTable.getData(this.filterColumnIndex, record.getTargetRow());
        ...
        if (this.writeTimestampFilterEnabled) {
            ...
            Long targetWriteTimeStamp = record.getPk().getWriteTimestamp();
            if (null==targetWriteTimeStamp) {
                return false;
            }
            if (targetWriteTimeStamp < minWriteTimeStampFilter
                    || targetWriteTimeStamp > maxWriteTimeStampFilter) {
